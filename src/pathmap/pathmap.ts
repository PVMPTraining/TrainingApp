// Purpose: Provides a list of all paths used in the application.

// Root Page
export const rootPagePath: string = "/";

// Home Page
export const homePagePath: string = "/home";

// Account Page
export const accountPagePath: string = "/account";

// Auth Pages
export const authPagePath: string = "/auth";
export const loginPagePath: string = "/auth/login";
export const signupPagePath: string = "/auth/signup";

// Fitness Pages
export const fitnessHomePagePath: string = "/fitness";
export const workoutPagePath: string = "/fitness/user-workouts";
export const exercisePagePath: string = "/fitness/exercises";
export const logWorkoutPagePath: string = "/fitness/log-workout";
export const fitnessToolsPagePath: string = "/fitness/tools";
export const liveWorkoutPagePath: string = "/fitness/log-workout/live";
export const newWorkoutPagePath: string = "/fitness/user-workouts/new-workout";
export const updateWorkoutPagePath: string = "/fitness/user-workouts/update-workout";

// Nutrition Pages
export const nutritionHomePagePath: string = "/nutrition";
export const nutritionToolsPagePath: string = "/nutrition/tools";

// Tools Pages
export const toolsHomePagePath: string = "/tools";
export const toolsFavoritesPagePath: string = "/tools/favorites";
