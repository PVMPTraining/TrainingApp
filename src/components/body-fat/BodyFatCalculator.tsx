...
const BodyFatCalculator: FC<BodyFatCalculatorProps> = ({}) => {
	...
	const BFPCalculateWithUSNavyFormulaHandler = (waist: number, neck: number, height: number, hip: number, gender: string) => {
	...
	const BFPCalculateWithBMIFormulaHandler = (weight: number, height: number, age: number, gender: string) => {
		let bodyFatPercentage: number | null = null;
	...
	const BFPCalculateFunctionsCallHandler = (values: FormDataTypes) => {
		...
		const bmiMethodResult = BFPCalculateWithBMIFormulaHandler(values.measurements.weight, values.measurements.height, values.age, values.gender);
				...
				{({ values, errors, handleChange }) => (
					<Form className="flex flex-col gap-5 items-center">
						<label className="flex flex-col items-center gap-1">
							...
							<div className="flex items-center gap-5">
								<label className="flex items-center gap-3">
									...
									<Field name="gender" type="radio" value="male" checked={values.gender === "male"} onChange={handleChange} />
								</label>
								<label className="flex items-center gap-3">
									<p className="text-white">Female</p>
									<Field name="gender" type="radio" value="female" checked={values.gender === "female"} onChange={handleChange} />
								</label>
								{errors.gender}
							</div>
						</label>
						<label className="flex flex-col gap-1">
							...
							<Field name="age" type="number" value={values.age === 0 ? "" : values.age} placeholder="Age" onChange={handleChange} />
							{errors.age}
						</label>
						<label className="flex flex-col gap-1">
							<p className="text-white">Weight</p>
							<Field name="measurements.weight" type="number" value={values.measurements.weight} placeholder="Weight" onChange={handleChange} />
							{errors.measurements?.weight}
						</label>
						<label className="flex flex-col gap-1">
							<p className="text-white">Height</p>
							<Field name="measurements.height" type="number" value={values.measurements.height} placeholder="Height" onChange={handleChange} />
							{errors.measurements?.height}
						</label>
						<label className="flex flex-col gap-1">
							<p className="text-white">Waist</p>
							<Field name="measurements.waist" type="number" value={values.measurements.waist} placeholder="Waist" onChange={handleChange} />
							{errors.measurements?.waist}
						</label>
						<label className="flex flex-col gap-1">
							<p className="text-white">Neck</p>
							<Field name="measurements.neck" type="number" value={values.measurements.neck} placeholder="Neck" onChange={handleChange} />
							{errors.measurements?.neck}
						</label>
						{values.gender === "female" ? (
							<label className="flex flex-col gap-1">
								<p className="text-white">Hip</p>
								<Field name="measurements.hip" type="number" value={values.measurements.hip} placeholder="Hip" onChange={handleChange} />
								{errors.measurements?.hip}
							</label>
						) : null}
						<button type="submit" className="bg-slate-700 px-4 py-3 text-white">
							Calculate
						</button>
					</Form>
				)}
			...
			{userResults.NavyMethod ? (
				<div className="text-white">
					...
					<div className="flex items-center gap-5">
						<div>
							<p>U.S Navy Method</p>
							<p className="text-white text-center text-lg">{userResults.NavyMethod.toFixed(2)}</p>
						</div>
						<div>
							<p>BMI Method</p>
							<p className="text-white text-center text-lg">{userResults.BMIMethod?.toFixed(2)}</p>
						</div>
					</div>
				</div>
			) : null}
